...
import com.demo.advancedmultimodulearchitecture.navigation.addComposableRoutes
...
fun AppScaffold(appNavigator: AppNavigator) {
  ...
  LaunchedEffect(navController) {
    appNavigator.destinations.collect { event ->
      when (event) {
        ...
        NavigatorEvent.PopBackStack -> navController.popBackStack()
      }
    }
  }
  ...
  Scaffold(modifier = Modifier.fillMaxSize()) { innerPadding ->
    NavHost(
      ...
      addComposableRoutes(appNavigator = appNavigator, navHostController = navController)
    }
  }
}
